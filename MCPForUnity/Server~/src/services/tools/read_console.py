...
async def read_console(
    ...
    types: (
        Annotated[
            list[Literal["error", "warning", "log", "all"]], "Message types to get"
        ]
        | None
    ) = None,
    count: (
        Annotated[
            int | str, "Max messages to return (accepts int or string, e.g., 5 or '5')"
        ]
        | None
    ) = None,
    filter_text: Annotated[str, "Text filter for messages"] | None = None,
    since_timestamp: (
        Annotated[str, "Get messages after this timestamp (ISO 8601)"] | None
    ) = None,
    format: (
        Annotated[Literal["plain", "detailed", "json"], "Output format"] | None
    ) = None,
    include_stacktrace: (
        Annotated[
            bool | str,
    ...
    params_dict = {
        k: v for k, v in params_dict.items() if v is not None or k == "count"
    }
