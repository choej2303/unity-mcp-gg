...
async def manage_asset(
    ...
    path: Annotated[
        str, "Asset path (e.g., 'Materials/MyMaterial.mat') or search scope."
    ],
    asset_type: (
        Annotated[
            str, "Asset type (e.g., 'Material', 'Folder') - required for 'create'."
        ]
        | None
    ) = None,
    properties: (
        Annotated[
            dict[str, Any] | str,
            "Dictionary (or JSON string) of properties for 'create'/'modify'.",
        ]
        | None
    ) = None,
    destination: Annotated[str, "Target path for 'duplicate'/'move'."] | None = None,
    generate_preview: Annotated[
        bool, "Generate a preview/thumbnail for the asset when supported."
    ] = False,
    search_pattern: Annotated[str, "Search pattern (e.g., '*.prefab')."] | None = None,
    ...
    page_number: (
        Annotated[int | float | str, "Page number for pagination"] | None
    ) = None,
) -> dict[str, Any]:
    ...
    def _parse_properties_string(raw: str) -> tuple[dict[str, Any] | None, str | None]:
        try:
            ...
            if not isinstance(parsed, dict):
                return (
                    None,
                    f"manage_asset: properties JSON must decode to a dictionary; received {type(parsed)}",
                )
            return parsed, "JSON"
            ...
            try:
                ...
                if not isinstance(parsed, dict):
                    return (
                        None,
                        f"manage_asset: properties string must evaluate to a dictionary; received {type(parsed)}",
                    )
                return parsed, "Python literal"
            except (ValueError, SyntaxError) as literal_err:
                return (
                    None,
                    f"manage_asset: failed to parse properties string. JSON error: {json_err}; literal_eval error: {literal_err}",
                )

    async def _normalize_properties(
        ...
        if isinstance(raw, dict):
            await ctx.info(
                f"manage_asset: received properties as dict with keys: {list(raw.keys())}"
            )
            return raw, None
        if isinstance(raw, str):
            await ctx.info(
                f"manage_asset: received properties as string (first 100 chars): {raw[:100]}"
            )
            ...
            if parsed is None:
                return None, source
            await ctx.info(
                f"manage_asset: coerced properties from {source} string to dict"
            )
            return parsed, None
        return (
            None,
            f"manage_asset: properties must be a dict or JSON string; received {type(raw)}",
        )
