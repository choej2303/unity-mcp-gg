...
async def run_tests(
    ctx: Context,
    mode: Annotated[
        Literal["EditMode", "PlayMode"], "Unity test mode to run"
    ] = "EditMode",
    timeout_seconds: (
        Annotated[int | str, "Optional timeout in seconds for the test run"] | None
    ) = None,
    test_names: (
        Annotated[
            list[str] | str,
    ...
    group_names: (
        Annotated[list[str] | str, "Same as test_names, except it allows for Regex"]
        | None
    ) = None,
    category_names: (
        Annotated[
    ...
    assembly_names: (
        Annotated[list[str] | str, "Assembly names to filter tests by"] | None
    ) = None,
) -> RunTestsResponse:
    ...
    response = await send_with_unity_instance(
        async_send_command_with_retry, unity_instance, "run_tests", params
    )
