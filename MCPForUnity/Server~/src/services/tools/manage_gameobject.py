...
from services.tools.utils import coerce_bool, coerce_vec3, parse_json_payload
...
async def manage_gameobject(
    ...
    action: (
        Annotated[
            ...
            "Perform CRUD operations on GameObjects and components.",
        ]
        | None
    ) = None,
    target: (
        Annotated[
            str,
            "GameObject identifier by name or path for modify/delete/component actions",
        ]
        | None
    ) = None,
    search_method: (
        Annotated[
            Literal[
                "by_id", "by_name", "by_path", "by_tag", "by_layer", "by_component"
            ],
    ...
    tag: (
        Annotated[
            str,
            "Tag name - used for both 'create' (initial tag) and 'modify' (change tag)",
        ]
        | None
    ) = None,
    parent: (
    ...
    position: (
        Annotated[
            ...
            "Position - [x,y,z] or string '[x,y,z]' for client compatibility",
        ]
        | None
    ) = None,
    rotation: (
        Annotated[
            Union[list[float], str],
            "Rotation - [x,y,z] or string '[x,y,z]' for client compatibility",
        ]
        | None
    ) = None,
    scale: (
        Annotated[
            Union[list[float], str],
            "Scale - [x,y,z] or string '[x,y,z]' for client compatibility",
        ]
        | None
    ) = None,
    components_to_add: (
        Annotated[list[str], "List of component names to add"] | None
    ) = None,
    ...
    components_to_remove: (
        Annotated[list[str], "List of component names to remove"] | None
    ) = None,
    ...
    new_name: (
        Annotated[str, "New name for the duplicated object (default: SourceName_Copy)"]
        | None
    ) = None,
    offset: (
        Annotated[
            Union[list[float], str],
            "Offset from original/reference position - [x,y,z] or string '[x,y,z]'",
        ]
        | None
    ) = None,
    ...
    distance: (
        Annotated[float, "Distance to move in the specified direction (default: 1.0)"]
        | None
    ) = None,
    world_space: (
        Annotated[
    ...
    position = coerce_vec3(position, default=position)
    rotation = coerce_vec3(rotation, default=rotation)
    scale = coerce_vec3(scale, default=scale)
    offset = coerce_vec3(offset, default=offset)
    ...
    if component_properties is not None and not isinstance(component_properties, dict):
        return {
            "success": False,
            "message": "component_properties must be a JSON object (dict).",
        }
